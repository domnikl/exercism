defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, timestamp} <- enter_building(now),
         ...
         {:ok, checksum} <- stamp_form(timestamp, counter_fn.(birthday), form) do
      {:ok, get_new_passport_number(timestamp, counter_fn.(birthday), checksum)}
    else
      {:coffee_break, _reason} -> {:retry, NaiveDateTime.add(now, 15 * 60)}
